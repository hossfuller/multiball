...
def trim_skeet_text(
    ...
    ## How did a skeet_list with only one element get in?
        ...
        allowed = max(
            const.SKEETS_CHAR_LIMIT - total_char_length - number_of_newlines, 0
        )
...
def write_desc_skeet_text(
    ...
    ## -------------------------------------------------------------------------
    if (
        "final_score" in game["home"]
        and game["home"]["final_score"] is not None
        and game["away"]["final_score"] is not None
    ):
        #fmt: off
    ...
    if total_skeet_length > const.SKEETS_CHAR_LIMIT:
        raise Exception(
            f"Basic skeet text is too long! (len = {total_skeet_length}):\n--\n{total_skeet_str}\n--\n"
        )
    ...
    if len(event) > 0:
        skeet_file_path = os.path.join(
            skeet_dir, f"{game['game_pk']}_{event['play_id']}_desc.txt"
        )
